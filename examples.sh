...
istio_ambient_with_kgateway_and_eastwest() {
  ...
  create_namespace "$_cluster1" "$_cluster1" ingress-gateways
  install_kgateway_ingress_gateway "$_context" ingress-gateway istio-gateways '*.example.com' 80
  install_kgateway_httproute "$_context" ingress-gateway istio-gateways helloworld helloworld 8001 helloworld.example.com
  install_kgateway_reference_grant "$_context" 
}
